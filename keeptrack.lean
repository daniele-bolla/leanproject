...
/-! Topologis sine curve is connected but not path-connected -/
...
lemma xcoord_pathContinuous (h_path : unitInterval → ℝ×ℝ) (hCont : Continuous h_path) : Continuous (fun t => (h_path t).1) :=
...
theorem T_is_not_path_conn : ¬ (IsPathConnected T)  := by
  ...
  have h_t₁_ge_t₀_in_eps_delta_bound : ∃ t₁:unitInterval, t₁ > t₀  ∧ dist t₀ t₁ < δ := by
    ...
    have t₀_lt_one : t₀ < 1 := by
      ...
      · intro h_eq_one
        have w_x_path : xcoord_path 1 = w_x := by
          simp only [xcoord_path, Function.comp_apply]
          rw [h_path.target]
          simp only [w, sine_curve, w_x, Prod.fst]
    ...
    have dist_case : dist t₀ ⟨s₁, hs₁, h'⟩ < δ := by
      ...
      linarith
    constructor
    · simp only [gt_iff_lt, s₁,s₀, h_path, xcoord_path,← Subtype.coe_lt_coe, Subtype.coe_mk]
      apply lt_min
      · exact (lt_add_iff_pos_right _).mpr (half_pos hδ)
      · exact t₀_lt_one
    · simp only [dist_eq_norm, s₁, s₀, ← Subtype.coe_mk]
      exact dist_case
...
lemma sin_comp_inv_is_continuous_on_pos_real : ContinuousOn (fun x : ℝ => Real.sin (x⁻¹)) (pos_real) := by
  apply Real.continuous_sin.comp_continuousOn
  · exact inv_is_continuous_on_pos_real

lemma sine_curve_is_continuous_on_pos_real : ContinuousOn (sine_curve) (pos_real) :=
  ContinuousOn.prodMk continuous_id.continuousOn sin_comp_inv_is_continuous_on_pos_real

lemma pos_real_is_path_conn : IsPathConnected (pos_real) := by
...
lemma S_is_conn : IsConnected S := S_is_path_conn.isConnected

lemma Z_is_Conn : IsConnected Z := isConnected_singleton
